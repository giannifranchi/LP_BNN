...
from networks.weight_sampler import TrainableRandomDistribution
from utils.kl_divergence import *
from networks.base_bayesian_module import BayesianModule, BayesianRNN
...
def variational_estimator(nn_class):
    ...
    def sample_elbo(self,
        ...
        for _ in range(sample_nbr):
            ...
            loss += loss0
            #print('critrion loss :',loss0,'KL loss',self.nn_kl_divergence())
            loss += self.nn_kl_divergence() * complexity_cost_weight
            _, predicted = torch.max(outputs, 1)
            correct += predicted.eq(labels).cpu().sum()
        return loss / sample_nbr, correct/ sample_nbr
